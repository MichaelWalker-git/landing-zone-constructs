...
> {
  ...
  for (const item of bucketItems) {
    await s3ClientLoggingAccount.send(new DeleteObjectCommand({
...
async function getAllConfigFiles(props: {
  ...
  if (props.configRepositoryLocation === 'codecommit') {
    return getAllCodeCommitFiles(props);
  } else if (props.configRepositoryLocation === 's3') {
    return getAllS3Files(props);
  } else {
...
async function listAllOuChildren(
  ...
  do {
    const childrenResponse = await throttlingBackOff(() =>
      organizationsClient.send(new ListChildrenCommand({
        ChildType: childType,
        ParentId: parentId,
      })),
    );
...
async function assumeRole(
  ...
  const assumeRoleResults = await throttlingBackOff(() =>
    stsClient.send(new AssumeRoleCommand({
      RoleArn: roleArn,
      RoleSessionName: 'MetadataAssumeRoleSession',
    })),
  );
  return {
    AccessKeyId: assumeRoleResults.Credentials!.AccessKeyId,
    SecretAccessKey: assumeRoleResults.Credentials!.SecretAccessKey,
    SessionToken: assumeRoleResults.Credentials!.SessionToken,
    Expiration: assumeRoleResults.Credentials!.Expiration,
  };
}
