...
export async function handler(event: CloudFormationCustomResourceEvent): Promise<{ Status: string } | undefined> {
  ...
  switch (event.RequestType) {
    case 'Create':
      await createTgwAssociation(ec2Client, options);
      return {
        Status: 'SUCCESS',
      };
    case 'Update':
      const oldOptions = setOptions(event.OldResourceProperties, event.ServiceToken);
      await deleteTgwAssociation(ec2Client, oldOptions);
      await createTgwAssociation(ec2Client, options);
      return {
        Status: 'SUCCESS',
      };
    case 'Delete':
      await deleteTgwAssociation(ec2Client, options);
      return {
        Status: 'SUCCESS',
      };
  }
...
/**
 * Set TGW association options based on event
 * @param resourceProperties { [key: string]: any }
 * @param serviceToken string
 * @returns TgwAssociationOptions
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
function setOptions(resourceProperties: { [key: string]: any }, serviceToken: string): TgwAssociationOptions {
  return {
 ...
 */
async function setEc2Client(options: TgwAssociationOptions, solutionId?: string): Promise<EC2Client> {
  ...
  if (options.owningAccountId && options.owningRegion) {
    if (!options.roleName) {
      throw new Error(`Cross-account TGW association required but roleName parameter is undefined`);
    }
    //
    // Assume role via STS
 ...
 */
async function createTgwAssociation(ec2Client: EC2Client, options: TgwAssociationOptions): Promise<void> {
  console.log(
    `Associating attachment ${options.transitGatewayAttachmentId} to TGW route table ${options.transitGatewayRouteTableId}...`,
  );
  try {
 ...
 * @returns Promise<string>
 */
async function deleteTgwAssociation(ec2Client: EC2Client, options: TgwAssociationOptions): Promise<void> {
  console.log(
    `Disassociating attachment ${options.transitGatewayAttachmentId} from TGW route table ${options.transitGatewayRouteTableId}...`,
  );
  try {
    await throttlingBackOff(() =>
      ec2Client.send(
